...
import sparkz.crypto.TestingCommons
import sparkz.crypto.authds.{EmptyByteArray, LeafData}
import sparkz.crypto.authds.merkle.MerkleTree.InternalNodePrefix
import sparkz.crypto.hash.{Digest, Keccak256}
...
class MerkleTreeSpecification extends AnyPropSpec with ScalaCheckDrivenPropertyChecks with Matchers with TestingCommons {
  ...
  property("Batch proof generation by duplicated indices") {
    val d = (0 until 10).map(_ => LeafData @@ sparkz.utils.Random.randomBytes(LeafSize))
    val tree = MerkleTree(d)
  ...
  property("Batch proof generation by negative indices") {
    val d = (0 until 5).map(_ => LeafData @@ sparkz.utils.Random.randomBytes(LeafSize))
    val tree = MerkleTree(d)
