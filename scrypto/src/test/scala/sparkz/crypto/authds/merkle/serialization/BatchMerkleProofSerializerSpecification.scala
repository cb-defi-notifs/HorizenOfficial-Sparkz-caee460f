...
import sparkz.crypto.authds.merkle.{BatchMerkleProof, Leaf, MerkleTree}
import sparkz.crypto.authds.{LeafData, Side, TwoPartyTests}
import sparkz.crypto.hash.{Digest, Digest32, Keccak256}
...
class BatchMerkleProofSerializerSpecification extends AnyPropSpec
  ...
  property("Batch proof serialization + deserialization") {
    ...
    forAll(smallInt) { N: Int =>
      whenever(N > 0) {
        val d = (0 until N).map(_ => LeafData @@ sparkz.utils.Random.randomBytes(LeafSize))
  ...
  property(testName = "empty deserialization input") {
    val serializer = new BatchMerkleProofSerializer[D, HF]
    val res = serializer.deserialize(sparkz.utils.Random.randomBytes(2))
  ...
  property(testName = "invalid deserialization input") {
    val serializer = new BatchMerkleProofSerializer[D, HF]
    val res = serializer.deserialize(sparkz.utils.Random.randomBytes(9))
    ...
    forAll(smallInt) { N: Int =>
      whenever(N > 0) {
        ...
        val proofs: Seq[(Digest, Side)] = (0 until N)
          .map(_ => LeafData @@ sparkz.utils.Random.randomBytes(LeafSize))
